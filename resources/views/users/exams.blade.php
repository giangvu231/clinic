				...
				<tr class="main">
					...
					<?php 
						 ...
						 <?php 
    						...
    						$dt5 = $mt->subDays(5);    					
						 ?>
						@if($medical_bill->schedule_date < $dt5)
						<span name="request_date"><b>{{ $medical_bill->schedule_date }}</b></span>
						@elseif($medical_bill->schedule_date < $dt1 &&  $medical_bill->schedule_date > $dt5 )
						<span name="request_date" style="color: red"><b>{{ $medical_bill->schedule_date }}</b></span>
						@elseif($medical_bill->schedule_date < $today &&  $medical_bill->schedule_date > $dt1)
						<span name="request_date" style="color: blue"><b>{{ $medical_bill->schedule_date }}</b></span>
						@else
					...
					<td>
						<div name="device_name">{{ $medical_bill->birth_date }}</div>
					</td>
					<td>
						<div name="access_id">{{ $medical_bill->accession_number }}</div>
					</td>
					<td style="text-transform: uppercase; font-size: 10px;">
						@if($medical_bill->study_status == "3" || $medical_bill->study_status == "4")
						<div name="status_id">
					...
					<td style="padding: 5px;">
						...
						@if($medical_bill->study_status == "4")
							<a data-placement="bottom" data-toggle="tooltip" title="Tái khám" class="float-left" style="color: #2f4358;" href="{{ route('get.reorder.view', ['accession_number' => $medical_bill->accession_number]) }}"><i class="fas fa-notes-medical fa-lg" style="color: blue;"></i></a>							
						@endif   
						@if($medical_bill->study_status == "3" && $medical_bill->is_edited == "0")
						<a href="{{ route('del.medical_bill',['id'=>$medical_bill->id]) }}" onclick="return confirm('Bạn có chắc chắn xóa?')" title="Xóa khỏi hàng chờ"><i class="fa fa-trash" style="color: red; cursor: pointer;"></i></a>
						@endif 
						@if($medical_bill->is_edited == "1" && $medical_bill->study_status == "3")
					...
					<td>
						<div class="float-right tooltip-right">
						@if($medical_bill->study_status == "4")
						<a data-placement="bottom" data-toggle="tooltip" title="Chi tiết" class="float-left" style="color: #2f4358;" href="{{ route('get.description.view', ['accession_number' => $medical_bill->accession_number]) }}"><i class="fas fa-magic fa-lg" style="color: green;"></i></a>
						@elseif($medical_bill->study_status == "3")
						<div class="float-right tooltip-right">
						<a data-placement="bottom" data-toggle="tooltip" title="Chỉnh sửa" class="float-left" style="color: #2f4358;" href="{{ route('edit_par', ['id' => $medical_bill->id]) }}"><i class="fas fa-edit fa-lg"></i></a>
						</div>
						@endif	
						</div>
					</td>
